...
async def set_drop_watcher_status(watcher_id: int, status: str) -> None:
    async with _conn() as con:
        await con.execute("UPDATE drop_watchers SET status = $1, updated_at = $2 WHERE id = $3", status, time.time(), watcher_id)
...
async def get_event_blocks(blocks_id: int) -> Optional[dict[str, Any]]:
    async with _conn() as con:
        row = await con.fetchrow("SELECT * FROM event_blocks WHERE id = $1", blocks_id)
        if not row: return None
        d = dict(row)
